		...
		if (pMem)
		{
				...
				(pMem->dwVersion >= 0x00020000))
			{
					...
					
				{
					...
					{
						RTSS_SHARED_MEMORY::LPRTSS_SHARED_MEMORY_OSD_ENTRY pEntry = (RTSS_SHARED_MEMORY::LPRTSS_SHARED_MEMORY_OSD_ENTRY)((LPBYTE)pMem + pMem->dwOSDArrOffset + dwEntry * pMem->dwOSDEntrySize);
						RTSS_SHARED_MEMORY::LPRTSS_SHARED_MEMORY_APP_ENTRY pAppEntry = (RTSS_SHARED_MEMORY::LPRTSS_SHARED_MEMORY_APP_ENTRY)((LPBYTE)pMem + pMem->dwAppArrOffset + (dwEntry-1) * pMem->dwAppEntrySize);
						...
						if (!strcmp(pEntry->szOSDOwner, "RTSSSharedMemorySample"))
						{
								...
								//instead of 256 for regular text slot
							{
									...
									//OSD locking is supported on v2.14 and higher shared memory
								{
									...
									{
										//char szProcessID[10];
										//sprintf_s(szProcessID, "%d", pAppEntry->dwProcessID);
										char szOut[100];
										sprintf_s(szOut, "%d", pAppEntry->dwProcessID, pAppEntry->szName);
										strncpy_s(pEntry->szOSDEx, sizeof(pEntry->szOSDEx), szOut/*lpText*/, sizeof(pEntry->szOSDEx) - 1);	
										pMem->dwBusy = 0;
										
										//MessageBox(pAppEntry->szName, szProcessID, NULL);
									}
								}
								else 
